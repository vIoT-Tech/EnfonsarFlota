...
function colissio(){
	...
	for (i; i<(h+2); i++){
	  for (k; k<(v+2); k++){
			...
			if ((document.getElementById([i]+[k]).value.substring(2,3) != 0) && (document.getElementById([i]+[k]).value.substring(2,3) != PesActual)) impossible++; // (0 || PesActual || null)  //statusfitxa // null si es fora del rang del tauler (00, 10, 20, 100)
		}
		}
