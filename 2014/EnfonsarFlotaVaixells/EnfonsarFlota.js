...
var PosActual="";
var PosDesitg="";
var PesActual=3; // PES del vaixell actual
...
function build(){// Posar l'imatge i el panell amb els botons de començar 
	for (h=1; h<=9; h++){
		...
		nouValor.setAttribute("value", [h]+[v]+"xy"); // Els 00 siginifica  no inicialitzada encara !
...
// (Possibles valors: idxI (Inicial) ; idxT (Tocat) ; idxE (Enfonsat) ; idxA (Aigua)  
// x = valor del pes acutal 2,3,4,5,6
function iniciar(){
...
function ocupat(){
	if (document.getElementById(PosDesitg).value.substring(2,3) == 'x') lliure=true;
	else lliure=false;
...
function colissio(){
	...
	for (i; i<(h+2); i++){
	  for (k; k<(v+2); k++){
			...
			console.log("colissio valor inicial de ik: " + [i]+[k]);
			console.log("([i]+[k])..substring(2,3):  " + document.getElementById([i]+[k]).value.substring(2,3));
			
			if (document.getElementById([i]+[k]).value.substring(2,3) != PesActual) // Fitxa ha està ocupada I A MËS no pertany al mateix tamany de vaixell propi 
				impossible = true; // (0 || PesActual || null)  //statusfitxa // null si es fora del rang del tauler (00, 10, 20, 100)
		}
		}
	  if (impossible==false) lliure=true; // si Segueix en = 0, vol dir que es podria ocupar amb una fitxa nova
	  else (lliure=false);
...
function colocar(){
	while (PesActual >= 2 && count <= PesActual) {
	 if (count <= PesActual) {
...
function reset(){
    build(); // WARNING això només afegeix més casselles a la pantalla
}